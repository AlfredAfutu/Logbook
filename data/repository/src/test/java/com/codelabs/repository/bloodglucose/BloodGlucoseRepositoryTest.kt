...
class BloodGlucoseRepositoryTest : RepositoryTest() {
    ...
    inner class Initialized {
        ...
        inner class BloodGlucoseAdded {
            ...
            @Test
            fun `then they should be stored in the database`() = runTest {
            ...
            inner class BloodGlucoseRetrieved {
                ...
                @Test
                fun `then it should return the correct number of readings`() = runTest {
                    Assertions.assertEquals(2, readings.size)
                }

                @Test
                fun `then it should return the first reading as inserted`() = runTest {
                    Assertions.assertEquals(129.0, readings[0].level)
                }

                @Test
                fun `then it should return the second reading as inserted`() = runTest {
